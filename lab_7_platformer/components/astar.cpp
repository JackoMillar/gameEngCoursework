...
//creating a list of nodes to visit which allows to traverse/move through
//the map using steering behaviours
//the node is a point in the search graph for the A* algorithm
...
public:
	...
	//priority is used to decide which node to choose next
	...
	{
		//euclidian distance (from lecture 14)
...
vector<Vector2i> pathFind(Vector2i start, Vector2i finish)
{
	...
	//Initialise the lookup maps
	//Initially everything looks traversable and
	...
	//Adding start node to the queue which will be checked first
	Node n0(start, 0, 0);
	...
	//then update the value in the open_nodes_map
	open_nodes_map[start.x][start.y] = n0.getPriority();


	//Loop until queue is empty, no more nodes to explore or the goal is reached
	while(!queue[queue_index].empty())
	{
		...
		if(pos == finish) 
		{
			...
			//Return the completed path as vector of positions
			return path;
		}
		//Not reached goal then check four directions
		for (auto& dir : directions) {
				...
				closed_nodes_map[next.x][next.y]))
			{
				...
				else if (open_nodes_map[next.x][next.y] > m0.getPriority())
				{
					...
					//Now replace the node by swapping across the lists 
					//and ignoring the old one
	...
	//if queue becomes empty without finding path to the finish node
